...
@Component({
  selector: 'app-process-assignment',
  templateUrl: './process-assignment.page.html',
  styleUrls: ['./process-assignment.page.scss'],
})
export class ProcessAssignmentPage implements OnInit {
  ...
  ngOnInit() {
    console.log('assign =>', this.assignment);
    this.selectors.org = this.assignment.org_id_src;
    this.form.quantity = this.assignment.quantity;
  }
    ...
    if (this.valid.formValid) {
      let newSequence = {
        ...
        asignacion_contribuyente_id:
          this.assignment.asignacion_contribuyente_id,
        endno: 0,
  ...
  saveAsignacion(_data: any) {
    const query = this.apiService.procesarAsignacionContribuyente(_data);
